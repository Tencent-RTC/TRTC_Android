 ...
 * <a href="https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__TRTCCloud__android.html#af7cf5544f9b8027e9526c32319a13838">
 *     setMixTranscodingConfig(TRTCCloudDef.TRTCTranscodingConfig config)</a>.
 * - To learn about the use cases and other details of On-Cloud MixTranscoding, see
 * <a href=https://cloud.tencent.com/document/product/647/16827">On-Cloud MixTranscoding</a>.
 */
public class PushCDNAnchorActivity extends TRTCBaseActivity
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.iv_back) {
            ...
            if (!mIsRoomEntered) {
                Toast.makeText(PushCDNAnchorActivity.this, R.string.pushcdn_anchor_need_enter_room_tip,
                        Toast.LENGTH_SHORT).show();
            } else if (mRemoteUidList.isEmpty()) {
                Toast.makeText(PushCDNAnchorActivity.this, R.string.pushcdn_anchor_more_anchor_tip, Toast.LENGTH_SHORT)
                        .show();
            } else {
            ...
            if (mIsRoomEntered) {
                ...
                if (TextUtils.isEmpty(mEditRoomId.getText())) {
                    Toast.makeText(PushCDNAnchorActivity.this, R.string.pushcdn_anchor_empty_room_id_tip,
                            Toast.LENGTH_SHORT).show();
                } else {
    ...
    public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
        ...
        if (compoundButton.getId() == R.id.rb_mix_mode_manual) {
            if (compoundButton.isChecked() && mMixConfigFlag != MixMode.MIX_MODE_MANUAL) {
                mMixConfigFlag = MixMode.MIX_MODE_MANUAL;
                setMixConfig();
            }
        } else if (compoundButton.getId() == R.id.rb_mix_mode_in_picture) {
            if (compoundButton.isChecked() && mMixConfigFlag != MixMode.MIX_MODE_PRESET_IN_PICTURE) {
                mMixConfigFlag = MixMode.MIX_MODE_PRESET_IN_PICTURE;
                setMixConfig();
            }
        } else if (compoundButton.getId() == R.id.rb_mix_mode_left_right) {
            if (compoundButton.isChecked() && mMixConfigFlag != MixMode.MIX_MODE_PRESSET_LEFT_RIGHT) {
            ...
            if (mRemoteUidList.size() > 0) {
                for (int i = 0; i < mRemoteUidList.size() || i < 3; i++) {
                    if (i < mRemoteUidList.size() && !TextUtils.isEmpty(mRemoteUidList.get(i))) {
                        ...
                        mTRTCCloud.startRemoteView(mRemoteUidList.get(i), TRTCCloudDef.TRTC_VIDEO_STREAM_TYPE_BIG,
